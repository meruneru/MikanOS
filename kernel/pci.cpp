...
Error ScanFunction(uint8_t bus, uint8_t device, uint8_t function){
    ...
    if(class_code.Match(0x06u, 0x04u)){ // PCI-to-PCI Bridge
        auto bus_numbners = ReadBusNumbers(bus, device, function);
        uint8_t secondary_bus = (bus_numbners >> 8) & 0xffu;
...
Error ScanBus(uint8_t bus){
    ...
    {
        if(ReadVenderId(bus, device, 0) == 0xffffu){ //function=0のVenderIDが無効値なら使われていないデバイス
            continue;
        }
...
bool IsSingleFunctionDevice(uint8_t header_type){
    return ((header_type & 0x80u) == 0); //7bit目が0なら単機能、1ならマルチファンクション
}


Error ScanAllBus(void){
    num_devices = 0;
